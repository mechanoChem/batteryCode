...
template <int dim>
void battery_porousModel<dim>::output_results (const unsigned int cycle) const
{
  ...
  data_out.attach_dof_handler (battery<dim>::dof_handler);
  //Add nodal DOF data
  data_out.add_data_vector (battery<dim>::Un, nodal_solution_names, DataOut<dim,hp::DoFHandler<dim> >::type_dof_data, nodal_data_component_interpretation);
