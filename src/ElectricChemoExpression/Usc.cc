...
void electricChemo<T,dim>::formula_Usc(T x, int Tdomainflag)
{
	if(Tdomainflag==-1){
		...
		l_Usc=c1+c2*exp(-c3*x)-c4*std::tanh((x-c5)/c6)-c7*std::tanh((x-c8)/c9)-c10*std::tanh((x-c11)/c12)-c13*std::tanh((x-0.99)/c14)-c15*x-c16*std::tanh((x-c17)/c18)-c19*std::tanh((x-c20)/c21);//
	...
	//positive electrode
	 ...
	 double b16= -8.6342730487746202;
		l_Usc = (b1+b2*x+b3*std::pow(x,2)+b4*std::pow(x,3)+b5*std::pow(x,4)+b6*std::pow(x,5)+b7*std::pow(x,6)+b8*std::pow(x,7)+b9*std::pow(x,8)+b10*std::pow(x,9))/(b11+b12*x+b13*std::pow(x,2)+b14*std::pow(x,3)+b15*std::pow(x,4)+b16*std::pow(x,5));
	}
}
